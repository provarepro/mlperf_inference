...
ARG BASE_IMAGE=openvino/${BASEOS_VER}_runtime:${OV_VER}
...
RUN CODE_PATH="closed/Intel/code/ssd-small/openvino-windows/ov_mlperf" && \
    git clone \
        --depth 1 \
        -b code \
        --single-branch \
...
RUN sed -i 's/${IE_EXTENSION_LIB}/${IE_RELEASE_LIBRARY}\n        IE::ie_cpu_extension/g' \
        ./CMakeLists.txt && \
    sed -i \
        -e 's/, sample_idxs,/, response_ids, sample_idxs));/' \
        -e '/response_ids));/d' \
        ./backend_ov.h && \
